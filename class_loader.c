...
void* parse_utf8(classloader_instance_t* instance, file_reader_t* reader){
    ...
    FAIL_SET_JUMP((ret = bumper_calloc(&instance->loader_arena,1, strlength + 1)),ret,NULL,exit);
...
void* parse_int(classloader_instance_t* instance, file_reader_t* reader){
    ...
    FAIL_SET_JUMP((ret = bumper_calloc(&instance->loader_arena,1,sizeof(java_int))),ret,NULL,exit);
...
void* parse_class(classloader_instance_t* instance, file_reader_t* reader){
    ...
    FAIL_SET_JUMP((class = bumper_calloc(&instance->loader_arena,1,sizeof(*class))),class,NULL,exit);
...
void* parse_string(classloader_instance_t* instance, file_reader_t* reader){
    ...
    FAIL_SET_JUMP((ret = bumper_calloc(&instance->loader_arena,1,sizeof(*ret))),ret,NULL,exit);
...
static classloader_error_t attribute_parse(classloader_instance_t* instance, file_reader_t* reader, classloader_attribute_t* output){
    ...
    FAIL_SET_JUMP((output->classloader_attribute = bumper_alloc(&instance->loader_arena,output->length)) || output->length == 0,err,CLASSLOADER_OOM,exit);
...
static classloader_error_t field_parse(classloader_instance_t* instance, file_reader_t* reader, classloader_field_t* output){
    ...
    FAIL_SET_JUMP(output->attributes_count == 0 || (output->attributes = bumper_calloc(&instance->loader_arena,output->attributes_count, sizeof(*output->attributes))),err,CLASSLOADER_OOM,exit);
...
classloader_instance_t* classloader_new(){
    bump_allocator_t classloader_arena = {0};
    int success = !bumper_create(&classloader_arena,CLASSLOADER_ARENA_SIZE);
    classloader_instance_t* instance = NULL;

    if(success){
        instance = bumper_alloc(&classloader_arena, sizeof(*instance));
        assert(instance);
...
classloader_error_t classloader_load_folder(classloader_instance_t* instance, const char* path){
    ...
    while ((in_file = readdir(dir_fd))) {
        if (strstr(in_file->d_name, ".class") && in_file->d_name[strlen(in_file->d_name) - 6] == '.' &&
            ...
            char* full_path = alloca(path_len + 1 + name_len + 1);
...
classloader_error_t classloader_load_class(classloader_instance_t* instance, file_reader_t* reader){
    ...
    classloader_class_t* java_class = bumper_calloc(&instance->loader_arena,1,sizeof(*java_class));
    ...
    FAIL_SET_JUMP((java_class->constants = bumper_calloc(&instance->loader_arena,java_class->constants_count - 1,
    ...
    FAIL_SET_JUMP(java_class->interfaces_count == 0 || (java_class->interfaces = bumper_calloc(&instance->loader_arena,java_class->interfaces_count,
...
//field parse code
    ...
    FAIL_SET_JUMP(java_class->fields_count == 0 || (java_class->fields = bumper_calloc(&instance->loader_arena,java_class->fields_count,
...
//method parse is the same, because field and method structures are same (at least in class loading stage)
    ...
    FAIL_SET_JUMP(java_class->methods_count == 0 || (java_class->methods = bumper_calloc(&instance->loader_arena,java_class->methods_count,
